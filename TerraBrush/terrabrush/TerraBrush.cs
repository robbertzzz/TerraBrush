...
public partial class TerraBrush : TerraBrushTool {
	...
	private string _rootPath = "res://TerraBrush/terrabrush/";
	...
	private async Task LoadTerrain() {
		...
		_terrain = (await AsyncUtils.LoadResourceAsync<PackedScene>(_rootPath + "Components/Terrain.tscn", CancellationToken.None)).Instantiate<Terrain>();
	...
	private async Task CreateFoliages() {
		...
		var prefab = await AsyncUtils.LoadResourceAsync<PackedScene>(_rootPath + "Components/Foliage.tscn", CancellationToken.None);
	...
	private async Task CreateWater() {
		...
		if (_waterNodeContainer == null) {
			...
			var prefab = await AsyncUtils.LoadResourceAsync<PackedScene>(_rootPath + "Components/Water.tscn", CancellationToken.None);
	...
	private async Task CreateSnow() {
		...
		var prefab = await AsyncUtils.LoadResourceAsync<PackedScene>(_rootPath + "Components/Snow.tscn", CancellationToken.None);
